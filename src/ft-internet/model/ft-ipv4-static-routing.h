...
#ifndef FT_IPV4_STATIC_ROUTING_H
#define FT_IPV4_STATIC_ROUTING_H
 ...
 */
class FtIpv4StaticRouting : public Ipv4RoutingProtocol
{
public:
  /**
 ...
 * \see Ipv4Address
 */
  void AddNetworkRouteTo (uint32_t flowId,
                          Ipv4Address network, 
                          Ipv4Mask networkMask, 
                          Ipv4Address nextHop, 
                          uint32_t interface,
...
/**
 * \brief Ft add a host route to the static routing table.
 *
 * \param flow id
 * \param dest The Ipv4Address destination for this route.
 * \param nextHop The Ipv4Address of the next hop in the route.
 ...
 * \see Ipv4Address
 */
  void AddHostRouteTo (uint32_t flowId,
                       Ipv4Address dest, 
                       Ipv4Address nextHop, 
                       uint32_t interface,
                       uint32_t metric = 0);
 ...
 */
  FtIpv4RoutingTableEntry GetDefaultRoute (void);
 ...
 */
  FtIpv4RoutingTableEntry GetRoute (uint32_t i) const;
 ...
 * \see Ipv4StaticRouting::RemoveRoute
 */
  FtIpv4MulticastRoutingTableEntry GetMulticastRoute (uint32_t i) const;
...
private:
  ...
  typedef std::list<std::pair <FtIpv4RoutingTableEntry *, uint32_t> > NetworkRoutes;

  /// Const Iterator for container for the network routes
  typedef std::list<std::pair <FtIpv4RoutingTableEntry *, uint32_t> >::const_iterator NetworkRoutesCI;

  /// Iterator for container for the network routes
  typedef std::list<std::pair <FtIpv4RoutingTableEntry *, uint32_t> >::iterator NetworkRoutesI;

  /// Container for the multicast routes
  typedef std::list<FtIpv4MulticastRoutingTableEntry *> MulticastRoutes;

  /// Const Iterator for container for the multicast routes
  typedef std::list<FtIpv4MulticastRoutingTableEntry *>::const_iterator MulticastRoutesCI;

  /// Iterator for container for the multicast routes
  typedef std::list<FtIpv4MulticastRoutingTableEntry *>::iterator MulticastRoutesI;
  ...
  /**
   * \brief Lookup in the forwarding table for destination with flow id.
   * \param flow id
   * \param dest destination address
   * \param oif output interface if any (put 0 otherwise)
   * \return Ipv4Route to route the packet to reach dest address
   */
  Ptr<Ipv4Route> LookupStatic (uint32_t flowId, Ipv4Address dest, Ptr<NetDevice> oif = 0);
